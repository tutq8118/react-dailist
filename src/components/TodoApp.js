...
function TotoItem (props) {
  return <div className={props.isCompleted ? 'TodoItem TodoItem-completed' : 'TodoItem'} onClick = {props.action} >
    {props.label}
  </div>
}
  ...
  render() {
    ...
    return (
      <div className='TodoApp'>
        ...
        {items.length > 0 && <main>
          <div className='TodoApp-List TodoApp-UpComming'>
            ...
            <div className='TodoApp-ListItems'>
              {items.filter((item) => item.isCompleted === false).map((item, index) => {
          ...
          <div className='TodoApp-List TodoApp-Finished'>
            ...
            {items.filter((item) => item.isCompleted === true).map((item, index) => {
                return <TotoItem action={this.ToggleComplete(item)} key={index} label={item.title} isCompleted={item.isCompleted} />
              })}
            </div>
          </div>
        ...
        {
          items.length === 0 && <main>
            Seems like You have no list..
          </main>
        }
        <footer>
          <button className='TodoApp-AddBtn' onClick={this.openModal}>+</button>
        </footer>
        {openModal && <Modal action={this.openModal} />}
      </div>
    )
  }
}
